...
for s in $tricky/ZN-rc $ITr/ZN-rc; do find "$s" | echo "$(wc -l) $s"; done
echo "\$tricky/ZN-rc: $(date +%y%m%d-%H%M%S), $host -> TOSHIBA" >> $misc/ITr/last_update
read -p "Enter to begin the rsync ? "
rsync -irtv --delete $tricky/ZN-rc/ $ITr/ZN-rc
